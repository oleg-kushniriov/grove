...
// Uses tc.Ctx, tc.K8s, tc.Namespace, and tc.Workload.GetLabelSelector() for watch configuration.
func (t *updateTracker) start(tc *testctx.TestContext) error {
	watcherCtx, cancel := context.WithCancel(tc.Ctx)
	t.cancel = cancel

	watcher, err := tc.K8s.WatchPods(watcherCtx, tc.Namespace, metav1.ListOptions{
